    ...
    def build_report(self):
        ...
        if hasattr(self, 'forecast_result'):
            if self.forecast_result:
                ...
                x_col = forecast_df.columns[0]
                # Prefer well-known forecast value columns
                preferred_cols = ['SCORE', 'SCORES']
                try:
                    col_types = forecast_df.get_table_structure()
                except Exception:
                    col_types = {}
                y_col = None
                ...
                def _is_string_type(type_name):
                    ...
                    return ('VARCHAR' in type_name) or ('NVARCHAR' in type_name) or ('CLOB' in type_name)
                if y_col is not None:
                    y_type = col_types.get(y_col)
                    if _is_string_type(y_type):
                        fr_cast = forecast_df.select(x_col, (f'CAST("{y_col}" AS DOUBLE)', 'FORECAST_VALUE'))
                        tse.add_line_to_comparison_item("Forecast Result",
                                                        data=fr_cast,
                                                        x_name=x_col,
                                                        y_name='FORECAST_VALUE')
                    else:
                        tse.add_line_to_comparison_item("Forecast Result",
                                                        data=forecast_df,
                                                        x_name=x_col,
                    ...
                    # Fallback: choose the last numeric column ignoring non-value columns
                    numeric_markers = ('DOUBLE', 'DECIMAL', 'INTEGER', 'BIGINT', 'REAL', 'FLOAT')
                    ignore_names = {'CONFIDENCE', 'REASON_CODE', 'PLACEHOLDER_1', 'PLACEHOLDER_2'}
                    for col_name in reversed(forecast_df.columns):
                        if col_name in ignore_names or col_name == x_col:
                            continue
                        type_name = (col_types.get(col_name) or '').upper()
                        if any(m in type_name for m in numeric_markers):
                            y_col = col_name
                            break
                    if y_col is None:
                        # pick first non-x column
                        for col_name in forecast_df.columns:
                            if col_name != x_col and col_name not in ignore_names:
                                y_col = col_name
                                break
                        if y_col is None:
                            y_col = forecast_df.columns[-1]
                    tse.add_line_to_comparison_item("Forecast Result",
                                                    data=forecast_df,
                                                    x_name=x_col,
                                                    y_name=y_col)
