...
class _MLPMultiTaskBase(PALBase):#pylint:disable=too-many-instance-attributes
    ...
    def __init__(self,#pylint: disable=too-many-arguments, too-many-branches, too-many-statements, too-many-locals, too-many-positional-arguments
        ...
        self.hidden_layer_size = self._arg('hidden_layer_size', hidden_layer_size, (list, tuple))
        ...
        if self.param_values is not None:
            for idx in range(len(self.param_values)):
                ...
                if param[0] == 'hidden_layer_size':
                    param_value = ', '.join([str(x).replace('[', '"').replace(']', '"').replace('(', '"').replace(')', '"')#pylint:disable=line-too-long
                    for x in param[1]])
...
class MLPMultiTaskClassifier(_MLPMultiTaskBase):
    ...
    Examples
    --------
    >>> mlp = MLPMultiTaskClassifier(hidden_layer_size=[5,5,5], activation='tanh')
    >>> mlp.fit(data=train_data, key='ID', label=['Y1', 'Y2'])
    ...
    Parameter selection (model selection) example (please refer to the documentation of in model_selction):

    >>> mlp_mtc = MLPMultiTaskClassifier(num_epochs = 10)
    >>> pscv = ParamSearchCV(estimator=mlp_mtc,
                        param_grid={'hidden_layer_size': [(10,10), (5,5,5), (15,15,15)],
                                    'optimizer': ['adam', 'sgd'],
                                    'learning_rate': [0.001, 0.005, 0.01]
                                    },
                        search_strategy='grid',
                        train_control={'fold_num':5,
                                       'resampling_method':'cv',
                                       'random_state':1,
                                       'ref_metric':['accuracy']},
                        scoring='accuracy')
    >>> pscv.fit(data=df_train, label='CLASS', key='ID')

    output:

    >>> mlp_mtc.model_.collect()
    >>> mlp_mtc.optim_param_.collect()
    """
...
class MLPMultiTaskRegressor(_MLPMultiTaskBase):
    ...
    Examples
    --------
    >>> mlp = MLPMultiTaskRegressor(hidden_layer_size=[5,5,5], activation='leaky-relu')
    >>> mlp.fit(data=train_data, key='ID', label=['Y1', 'Y2'])
    ...
    Parameter selection (model selection) example:
    ...
    >>> mlp_mtr = MLPMultiTaskRegressor()
    >>> pscv = ParamSearchCV(estimator=mlp_mtr,
                        param_grid={'hidden_layer_size': [(10,10), (5,5,5), (15,15,15)],
                                    'optimizer': ['adam', 'sgd'],
                                    'learning_rate': [0.001, 0.005, 0.01]
                                    },
    ...
    >>> mlp_mtr.model_.collect()
    >>> mlp_mtr.optim_param_.collect()
    """
    resampling_method_list = ['cv', 'cv_sha', 'cv_hyperband', 'bootstrap',
                              'bootstrap', 'bootstrap_sha', 'bootstrap_hyperband']
    evaluation_metric_map = {'rmse':'RMSE'}
