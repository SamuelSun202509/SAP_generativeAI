...
class ScheduledExecution(object):#pylint:disable=too-many-public-methods, too-many-instance-attributes
    ...
    - create a scheduled execution of the `score()` method of a fitted hana-ml object (limited to the case
    ...
    connection_context : :class:`~hana_ml.dataframe.ConnectionContext`
        Specifies the valid connection to SAP HANA database.

    Attributes
    ----------
    connection_context : :class:`~hana_ml.dataframe.ConnectionContext`
        Representing the connection to SAP HANA database.

    current_user : str
        Representing the info of `CURRENT_USER` reflected by the connection to SAP HANA database.

    Examples
    --------
    Scenario : There is a dataset that has been updated continuously. Assuming that the dataset is stored
    in a table called 'EXPERIMENT_DATA_FULL_TBL' in SAP HANA database. We want to schedule the training of an HGBT model
    on this dataset at 8AM each monday, and having the latest HGBT model stored in a table called 'EXPERIMENT_MODEL_TBL'.
    ...
    The task is successfully created if no error is raised.
    We can then attach the prescribed schedule mentioned in the beginning of this section to
    ...
    """
    def __init__(self, connection_context, current_user=None):
        self.connection_context = connection_context
        self.current_user = current_user if current_user is not None else \
        connection_context.sql('SELECT CURRENT_USER FROM DUMMY;').collect().iloc[0,0]#pylint:disable=line-too-long
        ...
        self._disable_execution = False # for internal test only, do not change
    ...
    def create_task(self,#pylint:disable=too-many-positional-arguments
        ...
        task_owner : str, optional
            Specifies the task owner, whom must be granted the privilege to call the stored SQL procedure specified by
            ``proc_name``.
    ...
    def create_sequential_composite_task(self, task_id, subtasks, task_desc='', force=True, with_hierarchy=True):
        r"""
        ...
        ----------
        task_id : str
            Specifies the name of the composite task to be created.
            The name must not conflict with names of existing tasks..
        subtasks : list of str
            Specifies the list of subtask IDs to be included in the sequential composite task.
            The subtasks will be executed in the order as given in the list.
        task_desc : str, optional
        ...
        """
        #raise NotImplementedError("The creation of sequential composite task has not been implemented yet!")
        task_id = arg('task_id', task_id, str)
        if task_id.startswith("__SUBTASK__"):
            raise ValueError("task_id cannot start with __SUBTASK__")
        conn = self.connection_context
        if force is True:
            try:
                self.remove_task(task_id, force=force, with_hierarchy=with_hierarchy)
            except dbapi.Error as db_err:
                logger.warning(str(db_err))
                pass
        subtasks = arg('subtasks', subtasks, ListOfStrings)
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        task_info_tbl = '#SEQUENTIAL_COMPOSITE_TASK_INFO_TBL_' + task_id.upper() + f'_{unique_id}'
        subtasks = arg('subtasks', subtasks, (list, tuple))
        header = "DO\nBegin\n"
        header = header + 'DECLARE subtask_template TABLE("SEQUENTIAL_ORDER" INTEGER, "SUBTASK_ID" NVARCHAR(256));\n'#pylint:disable=line-too-long
        header = header  + 'DECLARE composite_task_info TABLE("TASK_ID" NVARCHAR(256),' +\
        ' "TASK_OWNER" NVARCHAR(256), "TASK_TYPE" NVARCHAR(64));\n'
        for idx, subtask in enumerate(subtasks):
            header = header + f':subtask_template.INSERT(({idx + 1}, \'{subtask}\'));\n'
        create_proc = f'CALL "_SYS_AFL"."AFLPAL_CREATE_SEQUENTIAL_COMPOSITE_TASK_PROC"("TASK_ID" => \'{task_id}\',' +\
        f' "TASK_DESC" => \'{task_desc}\', "SUBTASK_TEMPLATE_TAB" => :subtask_template,' +\
        ' "COMPOSITE_TASK_INFO" => composite_task_info);\n'
        exec_statement = header + create_proc
        select_statement = f'CREATE LOCAL TEMPORARY COLUMN TABLE "{task_info_tbl}" AS (SELECT * FROM :composite_task_info);\n'
        exec_statement = exec_statement + select_statement + 'END;'
        if self._disable_execution:
    ...
    def create_parallel_composite_task(self, task_id, subtasks, task_desc='',#pylint:disable=too-many-positional-arguments
                                       parallel_attribute='ALL', force=True, with_hierarchy=True):
        r"""
        ...
        task_desc : str, optional
            ...
            Defaults to empty string.
        parallel_attribute : str, optional
            Specifies the parallel attribute of the composite task. It can take either of the following values:
            - 'ALL' : the parallel composite task is considered successful only if all its subtasks are successful.
            - 'ANY' : the parallel composite task is considered successful if any of its subtasks is successful.
        ...
        """
        #raise NotImplementedError("The creation of parallel composite task has not been implemented yet!")
        task_id = arg('task_id', task_id, str)
        if task_id.startswith("__SUBTASK__"):
            raise ValueError("task_id cannot start with __SUBTASK__")
        conn = self.connection_context
        ...
        subtasks = arg('subtasks', subtasks, ListOfStrings)
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        parallel_attribute  = parallel_attribute.upper()
        if parallel_attribute not in ['ALL', 'ANY']:
            raise ValueError("The value of 'parallel attribute' must be either 'ALL' or 'ANY'.")
        parallel_info_tbl = '#PARALLEL_TASK_INFO_TBL_' + task_id.upper() + f'_{unique_id}'
        subtasks = arg('subtasks', subtasks, (list, tuple))
        header = "DO\nBegin\n"
        header = header + 'DECLARE parallel_subtask_template TABLE("SUBTASK_ID" NVARCHAR(256));\n'
        header = header  + 'DECLARE parallel_task_info TABLE("TASK_ID" NVARCHAR(256),' +\
        ' "TASK_OWNER" NVARCHAR(256), "TASK_TYPE" NVARCHAR(64), "PARALLEL_ATTRIBUTE" NVARCHAR(16));\n'
        for subtask in subtasks:
            header = header + f':parallel_subtask_template.INSERT((\'{subtask}\'));\n'
        create_proc = 'CALL "_SYS_AFL"."AFLPAL_CREATE_PARALLEL_COMPOSITE_TASK_PROC"(' +\
        f'"TASK_ID" => \'{task_id}\', "TASK_DESC" => \'{task_desc}\', ' +\
        f'"PARALLEL_ATTRIBUTE" => \'{parallel_attribute}\', "SUBTASK_TEMPLATE_TAB" => :parallel_subtask_template,' +\
        ' "COMPOSITE_TASK_INFO" => parallel_task_info);\n'
        exec_statement = header + create_proc
        select_statement = f'CREATE LOCAL TEMPORARY COLUMN TABLE "{parallel_info_tbl}" AS (SELECT * FROM :parallel_task_info);\n'
        exec_statement = exec_statement + select_statement + 'END;'
        if self._disable_execution:
            return exec_statement
        try:
            ...
            logger.exception(str(db_err))
            raise
        return conn.table(parallel_info_tbl)
    ...
    def query_task(self, task_id):
        ...
        """
        #raise NotImplementedError("Query of task info has not been implemented yet!")
        conn = self.connection_context
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        tabs = ['chain', 'def', 'param', 'log']
        tables = ['#TASK_' + f'{tab.upper()}_INFO_TBL_{unique_id}' for tab in tabs]
        header = "Do\nBegin\n"
        header = header + 'DECLARE task_chain_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"TASK_DESCRIPTION" NVARCHAR(1024), "PARENT_TASK_ID" NVARCHAR(256), ' +\
        '"TASK_TYPE" NVARCHAR(64), "SEQ_ORDER" INTEGER, "PARALLEL_ATTRIBUTE" NVARCHAR(16), ' +\
        '"CURRENT_JOB_ID" INTEGER, "CREATED_AT" TIMESTAMP);\n'
        header = header + 'DECLARE task_def_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"PROC_SCHEMA" NVARCHAR(256), "PROC_NAME" NVARCHAR(256));\n'
        header = header + 'DECLARE task_param_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"PARAM_NAME" NVARCHAR(256), "ARGUMENT_SCHEMA" NVARCHAR(256), "ARGUMENT" NCLOB, ' +\
        '"ARGUMENT_CATEGORY" NVARCHAR(16));\n'
        header = header + 'DECLARE task_log_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"SEQ" BIGINT, "LOG_TIMESTAMP" TIMESTAMP, "LOG_MESSAGE" NVARCHAR(5000));\n'
        exec_statement = f'CALL "_SYS_AFL"."AFLPAL_QUERY_TASK_PROC"("TASK_ID" => \'{task_id}\',' +\
        ' "TASK_CHAIN_INFO" => task_chain_info, "TASK_DEF_INFO" => task_def_info, ' +\
        '"TASK_PARAM_INFO" => task_param_info, "TASK_LOG_INFO" => task_log_info);\n'
        exec_statement = header + exec_statement
        select_statements = [f'CREATE LOCAL TEMPORARY COLUMN TABLE "{tables[i]}" AS (SELECT * FROM :task_{tabs[i]}_info);\n' \
        for i in range(len(tabs))]
        for select_statement in select_statements:
            exec_statement = exec_statement + select_statement
        exec_statement = exec_statement + 'END;'
        if self._disable_execution:
            return exec_statement
        try:
            with conn.connection.cursor() as cur:
            ...
            logger.exception(str(db_err))
            raise
        return tuple(conn.table(table) for table in tables)

    def remove_task(self, task_id, force=False, with_hierarchy=False):
        """
        Remove a task.
        ...
        with_hierarchy : bool, optional
            Specifies whether or not to remove all kinds of tasks owned by current user.
            Suitable when the target task is a composite task.
        ...
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        task_info_tbl = '#TASK_INFO_TBL_' + task_id.upper() + f'_{unique_id}'
        exec_proc = 'AFLPAL_REMOVE_TASK_PROC'
        if with_hierarchy:
            exec_proc = 'AFLPAL_REMOVE_TASK_WITH_HIERARCHY_PROC'
        header = "DO\nBEGIN\nDECLARE info "
        info_str = 'TABLE("TASK_ID" NVARCHAR(256), "TASK_OWNER" NVARCHAR(256), "PROC_SCHEMA" NVARCHAR(256), "PROC_NAME" NVARCHAR(256));\n'
        if with_hierarchy:
            info_str = 'TABLE("TASK_ID" NVARCHAR(256), "TASK_OWNER" NVARCHAR(256), "TASK_TYPE" NVARCHAR(256), "PARALLEL_ATTRIBUTE" NVARCHAR(16), PROC_SCHEMA NVARCHAR(256), PROC_NAME NVARCHAR(256));\n'
        header = header + info_str
        call_statement = f'CALL "_SYS_AFL"."{exec_proc}"("TASK_ID" => \'{task_id}\', "TASK_INFO" => info);\n'
    ...
    def __exec_task_schedule(self, task_id, exec_type, cron, recurrence_range=None):
        ...
        if cron is not None:
            ...
            if len(freq_patterns) != len(frequency_keys):
                msg = "Incorrect 'cron' value specified."
                raise ValueError(msg)
    ...
    def create_task_schedule(self, task_id, cron, recurrence_range=None, force=False):
        ...
        recurrence_range : dict, optional
            ...
            For specifying the start or end points of the recurrence range (or both),
            user should always use string timestamp of the format "YYYY-MM-DD HH24:MI:SS.FF7", or a
    ...
    def query_task_schedule(self, task_id, last_days=0):
        r"""
        ...
        """
        #raise NotImplementedError("Cannot query the schedule info of the scheduled task since it is not implemented.")
        conn = self.connection_context
        task_id = arg('task_id', task_id, str)
        if task_id.startswith("__SUBTASK__"):
            raise ValueError("task_id cannot start with __SUBTASK__")
        ...
        tables = ['#TASK_' + f'{tab.upper()}_INFO_TBL_{unique_id}' for tab in tabs]
        header = "Do\nBegin\n"
        header = header + 'DECLARE task_schedule_info TABLE("SCHEDULE_NAME" NVARCHAR(256), ' +\
        '"CRON" NVARCHAR(256), "START_TIME" TIMESTAMP, "END_TIME" TIMESTAMP, ' +\
        '"IS_ENABLED" NVARCHAR(5), "IS_VALID" NVARCHAR(5), "CREATE_TIME" TIMESTAMP);\n'
        header = header + 'DECLARE task_log_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"SEQ" BIGINT, "LOG_TIMESTAMP" TIMESTAMP, "LOG_MESSAGE" NVARCHAR(5000));\n'
        exec_statement = f'CALL _SYS_AFL.AFLPAL_QUERY_TASK_SCHEDULE_PROC("TASK_ID" => \'{task_id}\',' +\
        f' "LAST_DAYS" => {last_days}, "TASK_SCHEDULE_INFO" => task_schedule_info,' +\
    ...
    def query_schedule_job(self, task_id, job_id=0):
        r"""
        ...
        if task_id.startswith("__SUBTASK__"):
            raise ValueError("task_id cannot start with __SUBTASK__")
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        tabs = ['schedule_job', 'log']
        tables = ['#TASK_' + f'{tab.upper()}_INFO_TBL_{unique_id}' for tab in tabs]
        header = "Do\nBegin\n"
        header = header + 'DECLARE task_schedule_job_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"PARENT_TASK_ID" NVARCHAR(256), "TASK_TYPE" NVARCHAR(64), "SEQ_ORDER" INTEGER, ' +\
        '"PARALLEL_ATTRIBUTE" NVARCHAR(16), "STATUS" NVARCHAR(16), ' +\
        '"START_TIME" TIMESTAMP, "END_TIME" TIMESTAMP);\n'
        header = header + 'DECLARE task_log_info TABLE("TASK_ID" NVARCHAR(256), ' +\
        '"LOG_TYPE" NVARCHAR(256), "SEQ" BIGINT, "LOG_TIMESTAMP" TIMESTAMP, "LOG_MESSAGE" NVARCHAR(5000));\n'
        exec_statement = f'CALL _SYS_AFL.AFLPAL_QUERY_SCHEDULE_JOB_PROC("TASK_ID" => \'{task_id}\',' +\
        f' "JOB_ID" => {job_id}, "SCHEDULE_JOB_INFO" => task_schedule_job_info,' +\
        ' "TASK_LOG_INFO" => task_log_info);\n'
        exec_statement = header + exec_statement
        select_statements = [f'CREATE LOCAL TEMPORARY COLUMN TABLE "{tables[i]}" AS (SELECT * FROM :task_{tabs[i]}_info);\n' \
        for i in range(len(tabs))]
    ...
    def create_one_off_task_schedule(self, task_id):
        """
        ...
        try:
            with conn.connection.cursor() as cur:
                #execute_logged(cur, fn_statement, conn.sql_tracer, conn)
                params = cur.callproc("_SYS_AFL.AFLPAL_EXECUTE_ONE_OFF_SCHEDULE_JOB_PROC", params)
        except dbapi.Error as db_err:
            logger.exception(str(db_err))
            raise
    ...
    def create_fit_task(self,#pylint:disable=too-many-positional-arguments
                        obj,
                        fit_params,
                        task_id,
                        ...
                        force=True):
        r"""
        Create a task for the `fit()` method of an hana-ml object.
        ...
        force : bool, optional
             ...
             If set as True, it will firstly try to drop previously existing procedures with the same name as well as tasks
             with the same ``task_id``, then re-created.
        ...
        if force:
            self.connection_context.execute_sql(f"DROP PROCEDURE {(smart_quote(proc_name))}")
        ...
        with self.connection_context.connection.cursor() as cur:
            execute_logged(cur, sql, self.connection_context.sql_tracer, self.connection_context)
    ...
    def create_fit_schedule(self,#pylint:disable=too-many-positional-arguments
        ...
        we want to schedule the training of an HGBT model using the UnifiedClassification
        ...
        ...                                               force=True)
        """
        self.create_fit_task(obj=obj, fit_params=fit_params, task_id=task_id,
                              proc_name=proc_name, force=force,
                              output_table_names=output_table_names)
    ...
    def __create_inference_task(self,#pylint:disable=too-many-positional-arguments
                                inference_task,
                                obj,
                                inference_params,
                                task_id,
    ...
    def __create_inference_schedule(self,#pylint:disable=too-many-positional-arguments
                                    ...
                                    output_table_names=None,
                                    force=True):
        self.__create_inference_task(inference_task=inference_task,
                                     obj=obj,
        ...
        return self.create_task_schedule(task_id=task_id,
                                         ...
                                         recurrence_range=recurrence_range,
                                         force=force)
    def create_predict_task(self,#pylint:disable=too-many-positional-arguments
                            obj,
                            predict_params,
                            task_id,
                            proc_name=None,
                            ...
                            force=True):
        r"""
        Create a task for the `predict()` method of an hana-ml object.

        Parameters
        ----------
        obj : hana-ml object
           A hana-ml object (i.e. an instance of some hana-ml class) that with callable `predict()` method.
        ...
        force : bool, optional
             ...
             It set as True, it will firstly try to drop previously existing procedures with the same name as well as tasks
             with the same ``task_id``, then re-created.

            Defaults to True.
        """
        self.__create_inference_task(inference_task='predict',
                                     obj=obj,
    ...
    def create_score_task(self,#pylint:disable=too-many-positional-arguments
                          obj,
                          score_params,
                          task_id,
                          proc_name=None,
                          ...
                          force=True):
        r"""
        Create a task for the `score()` method of an hana-ml object.
        ...
        force : bool, optional
            ...
            Defaults to True.
        """
        self.__create_inference_task(inference_task='score',
                                     obj=obj,
    ...
    def create_predict_schedule(self,#pylint:disable=too-many-positional-arguments
        ...
        and a separate data for prediction is stored in table "CLS_DATA_TBL_PREDICT".
        we want to schedule the prediction of an HGBT model in UnifiedClassification
    ...
    def create_score_schedule(self,#pylint:disable=too-many-positional-arguments
        ...
        obj : hana-ml object
           ...
           .. code-block:: python
             ...
             from hana_ml.algorithms.pal.unified_classificationimport UnifiedClassification
             obj = UnifiedClassification(func='HybridGradientBoostingTree', n_estimators=100).fit(data=data, key=..., label=...)
