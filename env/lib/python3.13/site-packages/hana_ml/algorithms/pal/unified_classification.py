...
class UnifiedClassification(PALBase, _UnifiedClassificationReportBuilder, AMDPHelper):#pylint: disable=too-many-instance-attributes
    ...
    **kwargs : keyword arguments
    ...
    def fit(self,#pylint: disable=too-many-branches, too-many-statements, too-many-positional-arguments
        ...
        **kwargs : keyword arguments
        ...
        if self.massive is True:
            ...
            # Validate group_params keys against available data groups regardless of HANA execution mode.
    ...
    def score(self,#pylint:disable=too-many-positional-arguments
        ...
        # Always keep the key column available for PAL input; do not drop it even if it's also set as index.
        ...
        if not self.pivoted:
            if self.massive is True:
                ...
                if index is not None:
                    ...
                    key = self._arg('key', key, str, required=True)
                # Enforce that key must be a data column in massive mode
                if key is not None and key not in cols:
                    msg = "Please select key from {}!".format(cols)
                    logger.error(msg)
                    raise ValueError(msg)
                # Remove key from feature candidates to avoid duplicate column in materialization
            ...
            if features is None:
                features = cols
            # Always include key column for PAL input, even if it's also an index
            sel_cols = group_id + [key] + features + [label]
            data_ = data[sel_cols]
        else:
            data_ = data
