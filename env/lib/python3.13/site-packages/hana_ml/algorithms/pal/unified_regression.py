...
class UnifiedRegression(PALBase, _UnifiedRegressionReportBuilder):
    ...
    **kwargs : keyword arguments
    ...
    def __forecast(self,#pylint:disable=too-many-branches, too-many-statements
        ...
        try:
            ...
            if fcn == 'PREDICT':
                ...
                try_drop(conn, outputs_score)
            raise
        err_msg = conn.table(errmsg_tbl) if errmsg_tbl else None
        # Guard against None or missing error message table, especially during PREDICT in massive mode
        if not self._disable_hana_execution and err_msg is not None:
            try:
                collected = err_msg.collect()
                if not collected.empty:
                    row = err_msg.count()
                    for i in range(1, row+1):
                        warn_msg = "For group_key '{}'".format(collected[group_key][i-1]) +\
                                   " the error message is '{}'.".format(collected['MESSAGE'][i-1]) +\
                                   "More information could be seen in the 2nd return Dataframe!"
                        logger.warning(warn_msg)
            except Exception:
                # If error table is not available, skip logging gracefully
                pass
        if predict:
    ...
    def fit(self,
        ...
        # Drop constant feature columns for linear regression to avoid PAL errors
        if self.func == 'MLR' and features:
            constant_cols = []
            for col in list(features):
                try:
                    if data.select(col).distinct().count() <= 1:
                        constant_cols.append(col)
                except Exception:
                    pass
            if constant_cols:
                logger.warning("Dropping constant columns in features: %s", constant_cols)
