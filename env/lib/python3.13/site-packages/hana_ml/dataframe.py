...
class ConnectionContext(object):
    ...
    def has_table(self, table, schema=None):
        ...
        cnt_tab = -1
        if table.startswith("#"):
            connection_id_df = DataFrame(self, "SELECT SESSION_CONTEXT('CONN_ID') FROM DUMMY").collect()
            if connection_id_df is None or getattr(connection_id_df, 'empty', False) or connection_id_df.shape[1] == 0:
                return False
            connection_id = connection_id_df.iat[0, 0]
    ...
    def get_current_schema(self):
        ...
        'TEST'
        """
        # Robustly retrieve current schema; fallback to CURRENT_USER if empty
        try:
            df = DataFrame(self, "SELECT CURRENT_SCHEMA FROM DUMMY").collect()
            if df is not None and getattr(df, 'empty', False) is False and df.shape[1] > 0:
                current_schema = df.iat[0, 0]
        ...
        # Fallback: In SAP HANA, default schema is current user
        try:
            df_user = DataFrame(self, "SELECT CURRENT_USER FROM DUMMY").collect()
            if df_user is not None and getattr(df_user, 'empty', False) is False and df_user.shape[1] > 0:
                current_user = df_user.iat[0, 0]
        ...
        # As a last resort return None (callers should handle gracefully)
        return None
...
restricted by the keys of ``to_replace`` and ``value``.
    ...
    def info(self):
        """
        Show the information of DataFrame with column name, dtype, non-null count, and optionally memory usage.

        If the DataFrame maps to a HANA's physical table, show memory usage (bytes) from the value of MEMORY_SIZE_IN_TOTAL of M_CS_TABLES.
        MEMORY_SIZE_IN_TOTAL displays that the total memory size, in bytes, is the sum of memory size in the main, delta, and history parts.

        """
        # Column dtypes and non-null counts
        dtype_map = {c[0]: c[1] for c in self.dtypes()}
        non_null_exprs = [f"COUNT(\"{col}\") AS \"{col}\"" for col in self.columns]
        non_null_sql = f"SELECT {', '.join(non_null_exprs)} FROM ({self.select_statement})"
        non_null_counts = self.connection_context.sql(non_null_sql).collect().iloc[0].to_dict()

        # Determine whether to display memory usage and fetch bytes from M_CS_TABLES
        show_memory = False
        mem_bytes = None
        table_meta = getattr(self, 'source_table', None)
        if isinstance(table_meta, dict):
            schema_name = table_meta.get('SCHEMA_NAME')
            table_name = table_meta.get('TABLE_NAME')
            # Skip temporary tables
            if isinstance(table_name, str) and not table_name.startswith('#'):
                try:
                    res = self.connection_context.sql(
                        f"SELECT MEMORY_SIZE_IN_TOTAL FROM M_CS_TABLES "
                        f"WHERE SCHEMA_NAME='{schema_name}' AND TABLE_NAME='{table_name}'"
                    ).collect()
                    if not res.empty:
                        v = res['MEMORY_SIZE_IN_TOTAL'].iat[0]
                        if pd.notna(v):
                            try:
                                mem_bytes = int(float(v))
                                show_memory = True
                            except Exception:
                                show_memory = False
                except Exception:
        ...
        info_rows = [{
            'column': col,
            'dtype': dtype_map.get(col, None),
            'non_null': non_null_counts.get(col, None)
        } for col in self.columns]
        info_df = pd.DataFrame(info_rows)
        dtype_counts = info_df['dtype'].value_counts().to_dict()

        print("<class 'hana_ml.dataframe.DataFrame'>")
        print(f"Rows: {row_num} entries")
        print(f"Data columns (total {len(info_df)} columns):")
        print(" #   Column         Non-Null Count         Dtype         ")
        print("---  ------         --------------         -----         ")
        for i, row in info_df.iterrows():
            print(f"{i:<3}  {row['column']:<14} {row['non_null']} non-null             {row['dtype']:<10}")

        dtype_strs = [f"{dtype}({count})" for dtype, count in sorted(dtype_counts.items())]
        print(f"dtypes: {', '.join(dtype_strs)}")
        if show_memory and mem_bytes is not None:
            # Show human-readable size in bytes, KB, MB or GB
            if mem_bytes >= 1024 ** 3:
                human = f"{mem_bytes / (1024 ** 3):.2f} GB"
            elif mem_bytes >= 1024 ** 2:
                human = f"{mem_bytes / (1024 ** 2):.2f} MB"
            elif mem_bytes >= 1024:
                human = f"{mem_bytes / 1024:.2f} KB"
            else:
                human = f"{mem_bytes} bytes"
            print(f"memory usage: {human}")
