...
class ModelStorage(object): #pylint: disable=useless-object-inheritance
    ...
    **delete** models.
    ...
    def load_model_from_files(self, path, model_schema=None, use_temporary_table=True, force=False, table_structure=None):
        """
        ...
        for r_, _, f_ in os.walk(os.path.join(path, 'models')):
            for file in f_:
                if '.zip' in file:
                    base = file[:-4]
                    key_name = ("#" + base) if use_temporary_table else base
                    files[key_name] = os.path.join(r_, file)
                    base_to_key[base] = key_name
        # determine load order
        def _nat_key(s):
            import re
            s1 = s[1:] if s.startswith('#') else s
            return [int(t) if t.isdigit() else t.lower() for t in re.split(r'(\d+)', s1)]
        artifacts_tables = None
        try:
            artifacts_tables = js_dict.get('artifacts', {}).get('model_tables', None)
        except Exception:
            artifacts_tables = None
        if isinstance(artifacts_tables, (list, tuple)) and len(artifacts_tables) > 0:
            desired_keys = [base_to_key[b] for b in artifacts_tables if b in base_to_key]
            remaining = [k for k in files.keys() if k not in desired_keys]
            load_order = desired_keys + sorted(remaining, key=_nat_key)
        else:
            load_order = sorted(files.keys(), key=_nat_key)
        model_ = []
        def _build_table_structure(pandas_df, base_structure=None, max_varchar_len=5000):
            resolved = dict(base_structure) if base_structure else {}
            for col_name in pandas_df.columns:
                if col_name in resolved:
                    continue
                if pandas_df[col_name].dtype == object:
                    try:
                        max_len = pandas_df[col_name].dropna().astype(str).map(len).max()
                    except Exception:
                        max_len = None
                    if max_len is not None and max_len > max_varchar_len:
                        resolved[col_name] = 'NCLOB'
        ...
        for m_name in load_order:
            ...
            if isinstance(table_structure, dict):
                if m_name in table_structure and isinstance(table_structure[m_name], dict):
                    base_structure = table_structure[m_name]
                else:
                    base_structure = table_structure
            table_structure_for_table = _build_table_structure(pandas_df, base_structure)
            model_.append(
                create_dataframe_from_pandas(self.connection_context,
                                             table_name=m_name,
                                             pandas_df=pandas_df,
                                             schema=model_schema,
                                             table_structure=table_structure_for_table,
                                             force=force)
            )
    ...
    def import_model(self, path, model_schema=None, force=False, table_structure=None):
        ...
        for r_, _, f_ in os.walk(os.path.join(path, 'models')):
            for file in f_:
                if '.zip' in file:
                    base = file[:-4]
                    files[base] = os.path.join(r_, file)
                    if self.connection_context.has_table(base, schema=model_schema):
                        raise ModelExistingError("{}.{} already exists.".format(quotename(model_schema), quotename(base)))

        # determine import order
        def _nat_key(s):
            import re
            return [int(t) if t.isdigit() else t.lower() for t in re.split(r'(\d+)', s)]
        artifacts_tables = artifacts.get('artifacts', {}).get('model_tables', None)
        if isinstance(artifacts_tables, (list, tuple)) and len(artifacts_tables) > 0:
            load_order = [b for b in artifacts_tables if b in files]
            remaining = [k for k in files.keys() if k not in load_order]
            load_order = load_order + sorted(remaining, key=_nat_key)
        else:
            load_order = sorted(files.keys(), key=_nat_key)
        ...
        for m_name in load_order:
            ...
            create_dataframe_from_pandas(self.connection_context,
                                         ...
                                         force=force)
        artifacts['artifacts']['model_tables'] = load_order
    ...
    def display_model_report(self, name, version=None):
        ...
        if _has_ipython_display:
            _ipython_display(HTML(metadata['MODEL_REPORT']))
        else:
            print(metadata['MODEL_REPORT'])
    # ===== Private methods
