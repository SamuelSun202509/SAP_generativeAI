...
def text_tokenize(data, lang=None, enable_stopwords=None, keep_numeric=None, allowed_list=None, notallowed_list=None, enable_stemming=None, allowed_regex_list=None, notallowed_regex_list=None):
    """
    ...
    allowed_regex_list : list, optional
        A list of regular expressions. Tokens matching any of these expressions are retained.

    notallowed_regex_list : list, optional
        A list of regular expressions. Tokens matching any of these expressions are removed.
    ...
    allowed_regex_list = arg('allowed_regex_list', allowed_regex_list, list)
    notallowed_regex_list = arg('notallowed_regex_list', notallowed_regex_list, list)
    # 自定义序列化为 JSON 风格的字符串列表，但不对反斜杠进行二次转义
    def _serialize_str_list(raw_list):
        if not raw_list:
            return None
        # 保持每个元素原样包裹双引号，避免 json.dumps 导致 "\\" 形式
        return '[' + ', '.join(['"{}"'.format(s) if s is not None else 'null' for s in raw_list]) + ']'

    allowed_regex_str = _serialize_str_list(allowed_regex_list)
    notallowed_regex_str = _serialize_str_list(notallowed_regex_list)

    # 1.
    procedure_name = 'PAL_TEXT_TOKENIZE'
    unique_id = str(uuid.uuid1()).replace('-', '_').upper()
    conn = data.connection_context
    if not 'mock' in conn.connection.__str__().lower():
    ...
    param_rows = [('LANGUAGE', None, None, lang),
                  ...
                  ('ALLOWED_REGEX_LIST', None, None, allowed_regex_str),
                  ('NOTALLOWED_REGEX_LIST', None, None, notallowed_regex_str)]
...
def text_log_parse(data, thread_number=None, thread_ratio=None):
    """
    ...
    thread_ratio : float, optional
        Specifies the ratio of threads that can be used by this function.
        The range of this parameter is from 0 to 1, where 0 means only using one thread and 1 means using all available threads. Values outside this range result in an error.
    ...
    param_rows = [('THREAD_NUMBER', thread_number, None, None),
                  ('THREAD_RATIO', None, thread_ratio, None)]
    output_tbls = ['#PAL_LOG_PARSE_{}_TBL_{}'.format(name, unique_id)
                   for name in ['SENTENCES_TEMPLATE_RESULT', 'TEMPLATE_INFO_RESULT', 'EXTRA_RESULT']]
    ...
    sentences_tbl, template_info_tbl, extra_tbl = output_tbls
    return (conn.table(sentences_tbl), conn.table(template_info_tbl), conn.table(extra_tbl))
