...
from hana_ml._compat.pandas_compat import to_str_list_compat, to_numpy
    ...
    for quarter, c in zip(labels, ['#1f77b4',  '#ff7f0e', '#2ca02c', '#9467bd']):
        for year in years:
            ...
            if enable_plotly:
                if not fig:
                    fig=go.Figure()
                fig.add_trace(go.Scatter(x=to_numpy(xx_filter_collect.iloc[:, 0]),
                                         y=to_numpy(xx_filter_collect.iloc[:, 1]),
                                         mode="lines", showlegend=False, marker=dict(size=10, color=c)))
            else:
                if ax is None:
                    ax = plt.axes()
                ax.plot(to_numpy(xx_filter_collect.iloc[:, 0]),
                    to_numpy(xx_filter_collect.iloc[:, 1]),
                        color=c)
...
def plot_time_series_outlier(data,
    ...
    if enable_plotly:
        trace_raw = go.Scatter(x=to_numpy(result[res_col[0]]), y=to_numpy(result[res_col[1]]), name=col)
        trace_outliers = go.Scatter(x=to_numpy(outliers[res_col[0]]),
                                    y=to_numpy(outliers[res_col[1]]), mode='markers',
    ...
    ax.plot(to_numpy(result[res_col[0]]), to_numpy(result[res_col[1]]), color='blue')
    ax.scatter(to_numpy(outliers[res_col[0]]), to_numpy(outliers[res_col[1]]), color='red')
    if title is None:
        title = "Outliers"
    ax.set_title(title)
    ax.set_xlabel(key)
...
def plot_change_points(data,
    ...
    if isinstance(cp_object, BCPD):
        ...
        if display_trend is True:
            ax.plot(to_numpy(components.collect()[key]), to_numpy(components.collect()["TREND"]), color='orange', label="Trend Component")

        ax.plot(to_numpy(data_.iloc[:, 0]), to_numpy(data_.iloc[:, 1]), color='blue', label="Original Time Series")
        if title is None:
            title = "Change Points"
        ax.set_title(title)
        ax.set_xlabel(key)
...
def plot_moving_average(data, col, rolling_window, key=None, ax=None, compare=True,
    ...
    if ax is None:
        ax = plt.axes()
    if compare:
        ax.plot(to_numpy(data_.iloc[:, 0]), to_numpy(data_.iloc[:, 1]), label=data_.columns[1])
    ax.plot(to_numpy(data_.iloc[:, 0]), to_numpy(data_.iloc[:, 2]), label=data_.columns[2])
    ax.legend()
...
def plot_rolling_stddev(data, col, rolling_window, key=None, ax=None,
    ...
    if enable_plotly:
        trace_sd = go.Scatter(x=to_numpy(data_.iloc[:, 0]), y=to_numpy(data_.iloc[:, 2]), name=data_.columns[2], showlegend=True)
...
def plot_seasonal_decompose(data,
    ...
    x_data = to_numpy(res.collect()[res.columns[0]])
    if enable_plotly:
        if fig is None:
            fig = make_subplots(rows=3, cols=1, shared_xaxes=True)
        for i in [1,2,3]:
            fig.add_trace(go.Scatter(x=x_data,
                                     y=to_numpy(res.select(res.columns[i]).collect()[res.columns[i]]),
                                     name=res.columns[i]),
                                     row=i,
                                     col=1)
    ...
    for num, color in zip(range(0, 3), ['b', 'r', 'g']):
        ax = axes[num]
        ax.plot(to_numpy(res.select(res.columns[0]).collect()[res.columns[0]]),
            to_numpy(res.select(res.columns[num + 1]).collect()[res.columns[num + 1]]),
...
def kdeplot(data, key, features=None, kde=kernel_density.KDE(), points=1000, enable_plotly=True, **kwargs):
    ...
    if len(columns) == 1:
        ...
        xmax = to_numpy(conn_context.sql(query).collect(geometries=False))[0][0]
        query = "SELECT MIN({}) FROM ({})".format(quotename(columns[0]), data.select_statement)
        xmin = to_numpy(conn_context.sql(query).collect(geometries=False))[0][0]
        ...
        ymax = to_numpy(conn_context.sql(query).collect(geometries=False))[0][0]
        query = "SELECT MIN({}) FROM ({})".format(quotename(columns[1]), data.select_statement)
        ymin = to_numpy(conn_context.sql(query).collect(geometries=False))[0][0]
...
class EDAVisualizer(Visualizer):
    ...
    def distribution_plot(self, data, column, bins, title=None, x_axis_fontsize=10, #pylint: disable= too-many-locals, too-many-arguments
        ...
        if self.enable_plotly:
            ...
            maxi = to_numpy(conn_context.sql(query).collect(geometries=False))[0][0]
            query = "SELECT MIN({}) FROM ({})".format(quotename(column), data_.select_statement)
            mini = to_numpy(conn_context.sql(query).collect(geometries=False))[0][0]
            if mini == maxi:
                maxi = mini + 1
            diff = maxi-mini
            ...
            if debrief:
                ...
                skewness = to_numpy(skewness.collect(geometries=False)['SKEWNESS'])[0]
                ax.text(max(bin_data.index)*0.9, max(to_numpy(bin_data['COUNT']))*0.95,
    ...
    def scatter_plot(self, data, x, y, x_bins=None, y_bins=None, title=None, label=None, #pylint: disable=too-many-locals, too-many-arguments, too-many-statements, invalid-name
        ...
        if self.enable_plotly:
            ...
            if x_bins is not None and y_bins is not None:
                ...
                x_maxi = to_numpy(conn_context.sql(x_max).collect(geometries=False))[0][0]
                x_min = "SELECT MIN({}) FROM ({})".format(quotename(x), data.select_statement)
                x_mini = to_numpy(conn_context.sql(x_min).collect(geometries=False))[0][0]
                ...
                y_maxi = to_numpy(conn_context.sql(y_max).collect(geometries=False))[0][0]
                y_min = "SELECT MIN({}) FROM ({})".format(quotename(y), data.select_statement)
                y_mini = to_numpy(conn_context.sql(y_min).collect(geometries=False))[0][0]
                ...
                if debrief:
                    ...
                    corr = to_numpy(data.corr(x, y).collect(geometries=False))[0][0]
        ...
        if self.enable_plotly:
            ...
            if list(aggregation.values())[0] == 'count':
                ...
                if len(bar_data.index) <= 20:
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.barh(xlabels, to_numpy(bar_data['COUNT']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                    ...
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.bar(xlabels, to_numpy(bar_data['COUNT']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                ...
                if len(bar_data.index) <= 20:
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.barh(xlabels, to_numpy(bar_data['AVG']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                    ax.set_ylabel(column, fontdict={'fontsize':label_fontsize})
                    ...
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.bar(xlabels, to_numpy(bar_data['AVG']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                    ax.set_xlabel(column, fontdict={'fontsize':label_fontsize})
                ...
                if len(bar_data.index) <= 20:
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.barh(xlabels, to_numpy(bar_data['MIN']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                    ax.set_ylabel(column, fontdict={'fontsize':label_fontsize})
                    ...
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.bar(xlabels, to_numpy(bar_data['MIN']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                    ax.set_xlabel(column, fontdict={'fontsize':label_fontsize})
                ...
                if len(bar_data.index) <= 20:
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.barh(xlabels, to_numpy(bar_data['MAX']), **kwargs)
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                    ax.set_ylabel(column, fontdict={'fontsize':label_fontsize})
                    ...
                    ax = self.ax #pylint: disable=invalid-name
                    xlabels = to_str_list_compat(bar_data[column])
                    ax.bar(xlabels, to_numpy(bar_data['MAX']), **kwargs)
                    #for item in ([ax.xaxis.label] + ax.get_xticklabels()):
                    for item in [ax.xaxis.label] + ax.get_xticklabels():
                        item.set_fontsize(10)
                    ax.set_xlabel(column, fontdict={'fontsize':label_fontsize})
        ...
        if self.enable_plotly:
            ...
            if groupby is None:
                ...
                if outliers:
                    ...
                    for i in to_numpy(suspected_outliers.collect(geometries=False)):
                        if n == 0:
                            if vert:
                    ...
                    n = 0 #pylint: disable=invalid-name
                    for i in to_numpy(outliers.collect(geometries=False)):
                        if n == 0:
                            if vert:
                ...
                values = conn_context.sql(query).collect(geometries=False)
                values = [i[0] for i in to_numpy(values)]
                ...
                for i in values:
                    ...
                    suspected_outliers.append(list(to_numpy(conn_context.sql(query).collect(geometries=False))))
                    query = "SELECT DISTINCT({}) FROM ({}) ".format(quotename(column),
                                                                    data_groupby.select_statement)
                    query += "WHERE {} > {} ".format(quotename(column), upp_out_fence)
                    query += "OR {} < {}".format(quotename(column), low_out_fence)
                    outliers_pt.append(list(to_numpy(conn_context.sql(query).collect(geometries=False))))
                n = 0 #pylint: disable=invalid-name
...
class Profiler(): # pragma: no cover
    ...
    def description(self, data, key, bins=20, missing_threshold=10, card_threshold=100, #pylint: disable=too-many-locals, too-many-arguments, too-many-branches, too-many-statements, no-self-use
        ...
        for i in data.columns:
            ...
            pct_missing = to_numpy(pct_missing.collect(geometries=False))[0][0]
        ...
        for i in data.columns:
            ...
            cardinality = to_numpy(cardinality.collect(geometries=False))[0][0]
