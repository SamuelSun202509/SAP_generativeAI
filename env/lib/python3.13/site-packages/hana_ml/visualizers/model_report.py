...
class StatisticReportBuilder(object):
    ...
    def __build_tables(self):
        ...
        for i in range(0, len(self.__statistic_table.columns)):
            col = self.__statistic_table.iloc[:, i]
            self.__column_data_map[self.__statistic_table.columns[i]] = to_str_list_compat(col)
...
class ParameterReportBuilder(object):
    ...
    def build_items(self):
        ...
        for i in range(0, len(self.__parameter_table_columns)):
            col = self.__parameter_table.iloc[:, i]
            table1_item.addColumn(
                self.__generated_table_columns[i],
                to_str_list_compat(col)
            )
    ...
    def get_item(self, title="Comparison"):
        ...
        for config in self.line_configs:
            ...
            try:
                ...
                x = to_str_list_compat(result[config[2]])
                y = []
    ...
    def _render_report(self):
        ...
        if self.framework_version == 'v2':
            ...
            if self.__statistic_table:
                ...
                table_item.addColumn('STAT NAME', to_str_list_compat(table_data.iloc[:, 0]))
                table_item.addColumn('STAT VALUE', to_str_list_compat(table_data.iloc[:, 1]))
                page = Page('Statistic')
