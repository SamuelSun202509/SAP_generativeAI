...
# pylint: disable=protected-access, bare-except, unnecessary-dunder-call, unsubscriptable-object
import logging
import os
import sys
import uuid
...
class EmbeddedUI(object):
    ...
    def get_runtime_platform():  # -1: console | 1: jupyter | 2: vscode | 3: bas | 4: databricks
        ...
        if str(sys.__dict__['displayhook']).find('<built-in') >= 0:
            ...
            is_exist_key_word1 = False
            is_exist_key_word2 = False
            for k in os.environ:
                if "ELECTRON_RUN_AS_NODE" in k:
                    is_exist_key_word1 = True
                elif "BAS_" in k:
                    is_exist_key_word2 = True
                elif "DATABRICKS" in k:
                    runtime_platform = (4, 'databricks')
                    break
            if is_exist_key_word1 and is_exist_key_word2:
                runtime_platform = (3, 'bas')
            elif is_exist_key_word1:
    ...
    @staticmethod
    def get_resource_root_file_path(file_name):
        if file_name is None:
            raise ValueError('The parameter file_name cannot be None!')
        return EmbeddedUI.get_resource_root_dir_path() + os.sep + file_name
    ...
    @staticmethod
    def print_tips_to_install_vscode_extension(action):
        print('In order to {} on the BAS or VSCode platform, you must import the VSCode extension package manually.'.format(action))
        print('VSCode extension package: {}'.format(EmbeddedUI.get_resource_root_file_path('hanamlapi-monitor-1.3.0.vsix')))
...
class ProcedureUtils(object):
    def __init__(self):
        pass

    @staticmethod
    def create_procedure(cursor: dbapi.Cursor, proc_name: str, proc_code: str, proc_parameter: str=''):
        procedure = '''
            CREATE PROCEDURE "{}"({})
            LANGUAGE SQLSCRIPT
            AS
            BEGIN
                {}
            END;
        '''.format(proc_name, proc_parameter, proc_code)
        try:
    ...
    @staticmethod
    def call_procedure(cursor: dbapi.Cursor, proc_name: str, parameters: List[Any]):
        try:
            cursor.callproc(proc_name, parameters)
            results = []
            fetched_data = cursor.fetchall()
            fetched_columns = [ column_desc[0] for column_desc in cursor.description ]
            results.append([fetched_data, fetched_columns])
    ...
    @staticmethod
    def delete_procedure(cursor: dbapi.Cursor, proc_name: str):
        try:
            cursor.execute('''
                DROP PROCEDURE "{}"
            '''.format(proc_name))
