...
from hana_ml._compat.pandas_compat import to_str_list_compat
...
def convert_sort_key(d):
    """Return a uniform comparable sort key for mixed x-axis labels.

    Priority:
    - numeric (int/float)
    - datetime/date/timestamp (as epoch seconds)
    ...
    if isinstance(d, date):
        dt_obj = datetime(d.year, d.month, d.day)
        return (1, dt_obj.timestamp())
    ...
    return (2, s.lower() if isinstance(s, str) else s)
...
class TimeSeriesExplainer(object):
    ...
    def set_forecasted_result_explainer(self, forecasted_result_explainer: dataframe.DataFrame, reason_code_name='EXOGENOUS'):
        ...
        for d in to_str_list_compat(self.forecasted_result_explainer_pd_df[self.key]):
    ...
    def get_comparison_item(self, title="Comparison"):
        ...
        for config in self.line_configs:
            ...
            try:
                ...
                for xd in to_str_list_compat(result[config[2]]):
        ...
        for config in self.confidence_interval_configs:
            ...
            if config[1] is not None:
                ...
                try:
                    ...
                    for d in to_str_list_compat(result[config[2]]):
                        d = convert_value_to_target_type(d)
                        x.append(d)
                    x_data.extend(x)
                    lower = []
        ...
        x_data.sort(key=convert_sort_key)
...
class DatasetAnalysis(object):
    ...
    def moving_average_item(self, rolling_window):
        ...
        X = []
        for xd in to_str_list_compat(data_.iloc[:, 0]):
            xd = convert_value_to_target_type(xd)
            push_valid_value(X, xd)
        X.insert(0, self.key)
        Y1 = []
    ...
    def timeseries_box_item(self, cycle=None):
        ...
        for xd in to_str_list_compat(temp_data_.iloc[:, 2]):
            xd = convert_value_to_target_type(xd)
            push_valid_value(X, xd)
        Y = []
    ...
    def seasonal_decompose_items(self, alpha=None, thread_ratio=None, decompose_type=None, extrapolation=None, smooth_width=None):
        ...
        for xd in to_str_list_compat(res.iloc[:, 0]):
            xd = convert_value_to_target_type(xd)
            push_valid_value(X, xd)
        X.insert(0, self.key)
        items = []
    ...
    def outlier_item(self, window_size=None, detect_seasonality=None, alpha=None, periods=None, outlier_method=None, threshold=None, **kwargs):
        ...
        for xd in to_str_list_compat(result.iloc[:, 0]):
            xd = convert_value_to_target_type(xd)
            push_valid_value(X, xd)
        x_data.extend(X)
        ...
        for xd in to_str_list_compat(outliers.iloc[:, 0]):
            xd = convert_value_to_target_type(xd)
            push_valid_value(X, xd)
        x_data.extend(X)
        X.insert(0, 'TIMESTAMP')
    ...
    def stationarity_item(self, method=None, mode=None, lag=None, probability=None):
        ...
        table_item.addColumn(columns[0], to_str_list_compat(stats.iloc[:, 0]))
        table_item.addColumn(columns[1], to_str_list_compat(stats.iloc[:, 1]))
        return table_item
    ...
    def real_item(self):
        ...
        X = []
        for xd in to_str_list_compat(data_[self.key]):
            xd = convert_value_to_target_type(xd)
            push_valid_value(X, xd)
        X.insert(0, self.key)
        Y = []
        ...
        if isinstance(cp_object, BCPD):
            ...
            if tcp.shape[0] > 0:
                ...
                if cp_style == "scatter":
                    ...
                    for xd in to_str_list_compat(result[title]):
                        xd = convert_value_to_target_type(xd)
                        push_valid_value(X, xd)
                    x_data.extend(X)
                    X.insert(0, title)
                ...
                if cp_style == "axvline":
                    result = tcp
                    result = to_str_list_compat(result.collect()[title])
            ...
            if display_trend is True:
                ...
                for xd in to_str_list_compat(result[self.key]):
                    xd = convert_value_to_target_type(xd)
                    push_valid_value(X, xd)
                x_data.extend(X)
                X.insert(0, self.key)
